...
<!-- FIX: map-fix.css must load AFTER leaflet.min.css to override Leaflet z-index values -->
<link rel="stylesheet" href="../css/map-fix.css">

<style>
/* Override main padding for full map */
...
/* MAP TOOLBAR — z-index set in map-fix.css (350) */
#map-toolbar {
  position: absolute;
...
/* MAP SIDE PANEL — z-index set in map-fix.css (360) */
#map-panel {
  width: 320px;
  background: var(--surface);
...
/* LEGEND — z-index set in map-fix.css (350) */
#legend {
  position: absolute;
...
/* COORDINATE DISPLAY — z-index set in map-fix.css (350) */
#coord-display {
  position: absolute;
  bottom: 8px;
  left: 50%;
...
/* COORDINATE INPUT PANEL — z-index set in map-fix.css (350) */
#coord-input-panel {
  position: absolute;
...
<!-- TOOLBARS (outside .main so no stacking conflict) -->
<div id="map-toolbar">
  ...
  <div class="map-tool-group">
    ...
    <button class="map-btn" id="btnProximity"  title="Proximity Analysis">⭕</button>
...
<!--
  FIX: Both modals are placed OUTSIDE #map-container and OUTSIDE .main.
  This prevents them from being trapped inside any CSS stacking context
  created by Leaflet's internal transform-based pane system.
  The .modal-overlay uses position:fixed so it already escapes normal flow,
  but an ancestor with CSS transform can still create a containing block
  that overrides fixed positioning in some browsers. Being outside #map
  entirely eliminates that risk.
-->
  ...
  <div class="modal" style="max-width:360px;">
    ...
    <div>
      ...
      <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:16px;">
        ...
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;">
          <input type="radio" name="basemap" value="satellite"> Satellite (Esri)
        </label>
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;">
      ...
      <div style="display:flex;flex-direction:column;gap:6px;">
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;">
          <input type="checkbox" id="layConsumers" checked> Water Meters / Consumers
        </label>
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;">
          <input type="checkbox" id="layPipelines" checked> Pipelines
        </label>
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;">
          <input type="checkbox" id="layInfra" checked> Infrastructure
        </label>
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;">
          <input type="checkbox" id="layParcels"> Parcel Borders
        </label>
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;">
          <input type="checkbox" id="layHeatmap"> Consumption Heatmap
        </label>
        <label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;">
          <input type="checkbox" id="layEmergency" checked> Emergency Incidents
        </label>
      </div>
    </div>
  </div>
...
async function loadConsumers() {
    ...
    try {
        ...
        (r.data || []).forEach(c => {
            ...
            m.bindPopup(buildConsumerPopup(c));
...
function buildConsumerPopup(c) {
    ...
    return `
        ...
        ${c.latest_consumption
            ? `<div class="popup-row"><span>Last Cons.</span><span>${parseFloat(c.latest_consumption).toFixed(2)} m³</span></div>`
            : ''}
        <div class="popup-actions">
            <button class="popup-btn popup-btn-info"
                onclick='openFeaturePanel("consumer", ${safe})'>View Details</button>
        </div>`;
}
...
async function loadPipelines() {
    ...
    try {
        ...
        (r.data || []).forEach(p => {
            ...
            const safeObj = JSON.stringify({ id: p.id, name: p.name, material: p.material, status: p.status })
                .replace(/"/g, '&quot;');
            line.bindPopup(`
                ...
                <div class="popup-actions">
                    <button class="popup-btn popup-btn-info"
                        onclick='openFeaturePanel("pipeline", ${safeObj})'>Details &amp; History</button>
                </div>`);
    ...
    try {
        ...
        (r.data || []).forEach(i => {
            ...
            m.bindPopup(`
                ...
                ${i.installation_date
                    ? `<div class="popup-row"><span>Installed</span><span>${i.installation_date}</span></div>`
                    : ''}`);
    ...
    try {
        ...
        (r.data || []).forEach(inc => {
            ...
            m.bindPopup(`
                ...
                <div class="popup-actions">
                    <button class="popup-btn popup-btn-warn"
                        onclick="resolveEmergency(${inc.id})">Mark Resolved</button>
                </div>`);
    ...
    if (type === 'consumer') {
        content.innerHTML = `
            ...
            <div class="card" style="margin-bottom:12px;">
                ...
                ${row('Last Consumption', data.latest_consumption
                    ? parseFloat(data.latest_consumption).toFixed(2) + ' m³' : '—')}
            </div>
        ...
        content.innerHTML = `
            ...
            <button class="btn btn-primary" style="width:100%;"
                onclick="loadPipelineHistory(${data.id})">View Change History</button>
...
document.getElementById('btnProximity').addEventListener('click', () => {
    ...
    document.getElementById('btnProximity').classList.toggle('active', proximityMode);
    showToast(proximityMode ? 'Click on the map to analyze proximity' : 'Proximity mode off', 'info');
});

map.on('click', async (e) => {
    if (!proximityMode) return;
    if (proximityCircle) map.removeLayer(proximityCircle);
    proximityCircle = L.circle(e.latlng, { radius: 500, color: '#ffb800', fillOpacity: 0.08, dashArray: '8,4' }).addTo(map);
    const r = await apiGet('gis.php', { action: 'proximity_analysis', lat: e.latlng.lat, lng: e.latlng.lng, radius: 0.5 });
    const panel   = document.getElementById('map-panel');
    const content = document.getElementById('panel-content');
    panel.classList.add('open');
    ...
    content.innerHTML = `
        <div class="card" style="margin-bottom:12px;">
            <div class="card-title">500m Radius</div>
            ${row('Lat', e.latlng.lat.toFixed(6))} ${row('Lng', e.latlng.lng.toFixed(6))}
        </div>
        <div class="card" style="margin-bottom:12px;">
            <div class="card-title">Infrastructure (${r.infrastructure?.length || 0})</div>
            ${(r.infrastructure || []).slice(0, 5).map(i =>
                `<div style="font-size:12px;padding:4px 0;border-bottom:1px solid var(--border);">
                    ${i.name || i.type} <span style="color:var(--muted);">${(i.distance_km * 1000).toFixed(0)}m</span>
                </div>`).join('')}
        </div>
        <div class="card">
            <div class="card-title">Consumers (${r.consumers?.length || 0})</div>
            ${(r.consumers || []).slice(0, 5).map(c =>
                `<div style="font-size:12px;padding:4px 0;border-bottom:1px solid var(--border);">
                    ${c.name} <span style="color:var(--muted);">${(c.distance_km * 1000).toFixed(0)}m</span>
                </div>`).join('')}
        </div>`;
});
...
document.getElementById('btnHeatmap').addEventListener('click', async () => {
    if (layerGroups.heatmap) {
        map.removeLayer(layerGroups.heatmap);
        layerGroups.heatmap = null;
